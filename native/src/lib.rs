...
use std::{collections::HashMap, ffi::CString, os::raw::c_char, str::FromStr, sync::Arc};
mod helpers;
...
lazy_static! {
    static ref CALLBACK_SUBSCRIPTIONS: Mutex<HashMap<PicoStreamingDevice, Arc<dyn NewDataHandler>>> =
        Default::default();
}
...
pub type DataCallback =
    extern "C" fn(channel_names: *const c_char, samples: *const f64, num_samples_per_ch: u32);
...
fn get_devices(download_missing_drivers: bool) -> Result<Vec<PicoDevice>> {
    ...
    Ok(devices.iter().map(|d| d.open().unwrap()).collect())
}
...
impl NewDataHandler for CallbackHandler {
    fn handle_event(&self, event: &StreamingEvent) {
        let mut data_vec = event
            .channels
            .iter()
        ...
        let concatenated_data: Vec<f64> = data_vec
            .into_iter()
            .map(|(_, samples)| samples)
            .flatten()
            .collect();

        let data_len = event.length;
...
pub extern "C" fn device_set_callback(
    ...
    catch_errors(device_ptr, false, |device| {
        let handler: Arc<dyn NewDataHandler> = Arc::new(CallbackHandler { callback });
        device.new_data.subscribe(handler.clone());

        CALLBACK_SUBSCRIPTIONS
            .lock()
            .insert(device.clone(), handler);
